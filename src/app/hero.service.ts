...
export class HeroService {
  ...
  updateHero(hero : Hero): Observable < any > {
    return this
      ...
      .pipe(tap(_ => console.log('tap')), tap(_ => this.log(`updated hero id=${hero.id}`)), catchError(this.handleError < any > ('updateHero')));
  }

  addHero(hero : Hero): Observable < any > {
    return this.http.post < Hero > (this.heroesUrl, hero, httpOptions).pipe(tap((hero: Hero) => this.log(`added hero w/ id=${hero.id}`)), catchError(this.handleError < Hero > (`addHero id=${hero.id}`)));
  }

  deleteHero(hero: Hero | number): Observable < any > {
    const id: number = typeof(hero) === 'number'
      ? hero
      : hero.id;
    const url = `${this.heroesUrl}/${id}}`;
